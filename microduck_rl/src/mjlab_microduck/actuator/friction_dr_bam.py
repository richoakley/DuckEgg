...
class FrictionDRBamActuator(BamActuator):
    ...
    def initialize(self, mj_model, model, data, device) -> None:
        ...
        # Per-joint output effectiveness for hidden deployment-fault evaluation.
        # This is one by default and therefore changes no existing task behavior.
        self.effort_scale = torch.ones(
            (self.kp_scale.shape[0], len(self._target_names)),
            dtype=self.kp_scale.dtype,
            device=self.kp_scale.device,
        )
        self.default_effort_scale = self.effort_scale.clone()
    ...
    def set_effort_scale(self, env_ids, effort_scale: torch.Tensor) -> None:
        """Set hidden motor-torque effectiveness by environment and joint."""

        self.effort_scale[env_ids] = effort_scale

    def reset_effort_scale(self, env_ids) -> None:
        self.effort_scale[env_ids] = self.default_effort_scale[env_ids]

    def compute(self, cmd: ActuatorCmd) -> torch.Tensor:
        motor_torque = super().compute(cmd)
        scaled = motor_torque * self.effort_scale
        # BAM's next-step voltage sag and friction load must see the torque that
        # reached the joint, not the unscaled internal proposal.
        if self._prev_motor_torque is not None:
            self._prev_motor_torque = scaled.detach()
        return scaled
...
class BacklashEncoderBamActuator(FrictionDRBamActuator):
    ...
    def get_command(self, data) -> ActuatorCmd:
        ...
        pos = (
            cmd.pos + data.joint_pos[:, self._backlash_joint_ids] * self._backlash_mask
        )
