...
The separate, forward-only EGGROLL experiment adapts an already-deployed PPO
actor against a measured non-differentiable deployment condition. It does not
replace PPO training. It includes actual-task CPU playback for deterministic
local A/B rehearsal, while calibration and training remain CUDA-only. See
[docs/eggroll_posttraining.md](docs/eggroll_posttraining.md).

The policy-agnostic continuation is [EGGROLL Policy Autopatch](docs/eggroll_autopatch.md):
one content-addressed campaign, actual-task evaluator, Rust runtime trace gate,
capability graph and reversible release path across all nine production policies.
