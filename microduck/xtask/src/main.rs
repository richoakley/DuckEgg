...
enum Command {
    ...
    /// Assemble a versioned policy bundle for one model updater component.
    PackageModel {
        /// Model release version. Models version independently of the daemon.
        #[arg(long)]
        version: semver::Version,

        /// Updater component/channel, for example `model-stand`.
        #[arg(long)]
        channel: String,

        /// ONNX policy to install as `policy.onnx`.
        #[arg(long)]
        policy: PathBuf,

        /// Auditable EGGROLL release manifest bound to this exact policy. Required for
        /// derivatives; omit only when packaging the production source as a rollback target.
        #[arg(long)]
        evidence_manifest: Option<PathBuf>,

        /// Production-loader verification referenced by the evidence manifest.
        #[arg(long)]
        runtime_verification: PathBuf,

        /// Where to write the artifact and unsigned updater manifests.
        #[arg(long, default_value = "dist")]
        out: PathBuf,

        /// Base URL the robot will download from.
        #[arg(long)]
        base_url: Option<String>,
        ...
        /// Sensor/actuator contract required by the model.
        #[arg(long, default_value_t = 1)]
        model_api: u32,

        /// Minimum hardware revision this model supports.
        #[arg(long, default_value_t = 0)]
        min_hw_rev: u32,

        /// zstd compression level. Lower only for throwaway tests.
        #[arg(long, default_value_t = 19)]
        zstd_level: i32,
    },
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    match Cli::parse().command {
        ...
        Command::PackageModel {
            ...
            min_hw_rev,
            zstd_level,
        } => package_model(ModelPackageArgs {
            version,
            channel,
            policy,
            evidence_manifest,
...
/// Package a policy through the same signed, atomic updater contract as daemon releases.
///
/// The evidence checks are intentionally done before writing any release files. A caller
/// cannot accidentally package a similarly named ONNX, or a policy whose compatibility was
/// only checked by Python: the manifest must bind the bytes and record a passing probe through
/// `duck_control::policy::Policy::load`, the loader used by `robotd`.
fn package_model(args: ModelPackageArgs) -> Result<(), Box<dyn std::error::Error>> {
    if !args.channel.starts_with("model-") {
        return Err(format!(
            "model channel must start with 'model-', got {:?}",
            args.channel
        )
        .into());
    }
    if args.model_api == 0 {
        return Err("model API must be at least 1".into());
    }

    let policy_bytes = std::fs::read(&args.policy)
        .map_err(|e| format!("reading {}: {e}", args.policy.display()))?;
    let policy_digest = sha256_hex(&policy_bytes);
    let runtime_bytes = std::fs::read(&args.runtime_verification)
        .map_err(|e| format!("reading {}: {e}", args.runtime_verification.display()))?;
    let runtime_digest = sha256_hex(&runtime_bytes);
    let runtime: serde_json::Value = serde_json::from_slice(&runtime_bytes)?;
    if runtime.get("passed").and_then(serde_json::Value::as_bool) != Some(true) {
        return Err("production loader verification did not pass".into());
    }
    if runtime
        .get("policy_sha256")
    ...
    {
        return Err("production loader probe is for different policy bytes".into());
    }
    if runtime.get("model_api").and_then(serde_json::Value::as_u64)
        != Some(u64::from(args.model_api))
    {
        return Err(format!(
            "production loader model API does not match --model-api {}",
            args.model_api
        )
        .into());
    }

    let evidence_bytes = if let Some(path) = &args.evidence_manifest {
        let bytes = std::fs::read(path).map_err(|e| format!("reading {}: {e}", path.display()))?;
        let evidence: serde_json::Value = serde_json::from_slice(&bytes)?;
        let evidence_digest = evidence
            .pointer("/adapted_policy/sha256")
            .and_then(serde_json::Value::as_str)
            .ok_or("evidence manifest has no adapted_policy.sha256")?;
        if evidence_digest != policy_digest {
            return Err(format!(
                "policy sha256 {policy_digest} does not match evidence manifest {evidence_digest}"
            )
            .into());
        }
        let runtime_reference = evidence
            .pointer("/runtime_contract/production_loader_verification")
            .ok_or("evidence manifest has no production loader verification")?;
        ...
        {
            return Err(
                "evidence manifest does not record a passing production loader probe".into(),
            );
        }
        if runtime_reference
            .get("sha256")
        ...
        {
            return Err("runtime verification sha256 does not match the evidence manifest".into());
        }
        Some(bytes)
    } else {
        None
    ...
    let artifact_name = format!("{}-{}.tar.zst", args.channel, args.version);
    let artifact = args.out.join(&artifact_name);
    let evidence_sha256 = evidence_bytes.as_deref().map(sha256_hex);

    {
        let file = std::fs::File::create(&artifact)?;
        let encoder = zstd::Encoder::new(file, args.zstd_level)?.auto_finish();
        let mut builder = tar::Builder::new(encoder);
        append_bytes(&mut builder, "policy.onnx", &policy_bytes, 0o644)?;
        if let Some(bytes) = &evidence_bytes {
            append_bytes(&mut builder, "eggroll-release.json", bytes, 0o644)?;
        }
        append_bytes(
            &mut builder,
        ...
        if let Some(digest) = &evidence_sha256 {
            version_toml.push_str(&format!("evidence_sha256 = \"{digest}\"\n"));
        }
        append_bytes(&mut builder, VERSION_FILE, version_toml.as_bytes(), 0o644)?;
        builder.finish()?;
        drop(builder);
    ...
    let mut manifest = serde_json::json!({
        ...
        "changelog": format!(
            "{}policy {} ({})",
            if evidence_bytes.is_some() { "EGGROLL " } else { "Production rollback " },
            args.policy.file_name().and_then(|name| name.to_str()).unwrap_or("policy.onnx"),
            &policy_digest[..12]
        ),
    });
    if let Some(revision) = &args.revision {
    ...
    local["sig_url"] = serde_json::json!(format!("{artifact_name}{SIG_SUFFIX}"));
    std::fs::write(
        args.out.join(format!("{}.manifest.json", args.version)),
        serde_json::to_vec_pretty(&local)?,
    )?;

    println!(
        "packaged model {} ({} bytes)",
        artifact.display(),
        artifact_bytes.len()
    );
    println!("  policy sha256 {policy_digest}");
    println!("  artifact sha256 {artifact_digest}");
    if let Some(digest) = evidence_sha256 {
        println!("  evidence sha256 {digest}");
    } else {
        println!("  evidence source production rollback target");
    }
    println!(
        "\nnext: cargo xtask sign --dir {} --key <key>",
        args.out.display()
...
#[cfg(test)]
mod tests {
    use super::{ModelPackageArgs, package_model, sha256_hex};

    #[test]
    fn model_package_binds_policy_evidence_and_runtime_contract() {
        let temp = tempfile::tempdir().unwrap();
        let policy = temp.path().join("adapted.onnx");
        let runtime_path = temp.path().join("runtime.json");
        let evidence_path = temp.path().join("evidence.json");
        let out = temp.path().join("dist");
        let policy_bytes = b"synthetic policy bytes";
        ...
        let evidence = serde_json::json!({
            "adapted_policy": { "sha256": policy_sha },
            "runtime_contract": {
        ...
        package_model(ModelPackageArgs {
            ...
            channel: "model-stand".into(),
            policy,
            evidence_manifest: Some(evidence_path),
        ...
        let updater_manifest: serde_json::Value =
            serde_json::from_slice(&std::fs::read(out.join("manifest.json")).unwrap()).unwrap();
        assert_eq!(updater_manifest["channel"], "model-stand");
        assert_eq!(updater_manifest["model_api"], 1);

        let archive = std::fs::File::open(out.join("model-stand-1.0.0.tar.zst")).unwrap();
    ...
    fn every_policy_in_the_repo_is_packaged() {
        let root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .expect("xtask/ has a parent");
        let mut policies: Vec<String> = std::fs::read_dir(root.join("example_policies"))
            .expect("example_policies/ must exist")
            .filter_map(|e| e.ok())
        ...
        for site in PACKAGING_SITES {
            ...
            for policy in &policies {
                let expected = format!("example_policies/{policy}=policies/{policy}");
                assert!(
                    text.contains(&expected),
                    "{site} does not package example_policies/{policy}. \
                     Add:  --include \"example_policies/{policy}=policies/{policy}\""
                );
            }
        }
    }
