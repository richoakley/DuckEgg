...
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct Step {
    /// Exact 61D input assembled by the deployed observation builder.
    pub observation: [f32; duck_control::OBS_LEN],
    /// Raw 14D ONNX output, before scaling or filtering.
    pub raw_action: [f32; ACTION_LEN],
    /// Home plus scaled raw action, before the production low-pass filters.
    pub unfiltered_targets: [f64; NUM_JOINTS],
...
impl Controller {
    ...
    pub fn step(
        ...
        let mut unfiltered_targets = [0.0; NUM_JOINTS];
        for joint in 0..NUM_JOINTS {
            unfiltered_targets[joint] = DEFAULT_POSITION[joint] + scale * offsets[joint];
        }
        let mut targets = unfiltered_targets;
