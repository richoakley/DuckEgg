...
struct Args {
    ...
    /// Run against a robot made of nothing. For laptop development and tests where
    /// physics is not required.
    #[arg(long)]
    fake: bool,

    /// Run against a MicroDuck body served from MuJoCo at `host:port`.
    ///
    /// The production controller, observation builder, ONNX loader, safety path,
    /// IPC and health reporting remain unchanged. Only the `RobotIo` transport is
    /// replaced. The link reconnects after simulator restarts.
    #[arg(long, conflicts_with = "fake")]
    sim: Option<String>,

    /// Start a simulator episode at its exact reset pose and immediately run the policy.
    ///
    /// This is evaluation-only orchestration for the production-runtime digital twin. It
    /// bypasses homing, seated-boot selection and the physical IMU warm-up wait, all of which
    /// would otherwise overwrite the StandUp task reset. Observation construction, ONNX
    /// inference, action processing, safety gates, timing and `RobotIo` writes are unchanged.
    #[arg(long, requires = "sim")]
    sim_eval: bool,

    /// Preserve the exact simulator reset but wait for an IPC driver to queue its
    /// command/skill and explicitly enable the policy. This removes the otherwise
    /// unavoidable first standing/walking tick from triggered capability evaluations.
    #[arg(long, requires = "sim_eval")]
    sim_eval_wait: bool,
...
fn spawn_control_thread(
    ...
    let sim_eval = args.sim_eval;
    let sim_eval_autostart = args.sim_eval && !args.sim_eval_wait;
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            // `RemoteIo` connects lazily and retries from the next control tick
            // after a simulator restart, matching the loop's existing tolerance
            // for a temporarily unavailable physical bus.
            ...
            runtime.block_on(async move {
                if let Some(io) = open_bus_waiting(&port, &state).await {
                    control_loop(io, state, intents, params, period, poweroff, false, false).await;
                }
            });
        })
}
        ...
        match Policy::load(&paths, DEFAULT_STANDING_THRESHOLD) {
            Ok(mut policy) => {
                ...
                tracing::warn!(
                    ...
                    walk_sha256 = ?policy_sha256(&policy_cfg.walk).ok(),
                    stand_sha256 = ?policy_cfg.stand.as_deref().and_then(|p| policy_sha256(p).ok()),
                    sitstand_sha256 = ?policy_cfg.sitstand.as_deref().and_then(|p| policy_sha256(p).ok()),
                    limp_fall,
                    "policy loaded"
                );
...
async fn control_loop<T: RobotIo>(
    ...
    let mut seated_boot = !sim_eval && leg_deviation > SEATED_BOOT_RAD;
    if seated_boot {
        tracing::warn!(
    ...
    if sim_eval {
        if let Err(e) = safety.set_torque(true) {
            tracing::error!(error = %e, "--sim-eval: cannot enable simulator torque");
            return;
        }
        if sim_eval_autostart {
            intents.set_enabled(true);
            tracing::warn!(
                "--sim-eval: preserving the task reset and starting the production policy pipeline"
            );
        } else {
            tracing::warn!(
                "--sim-eval-wait: preserving the task reset until an IPC driver enables the policy"
            );
        }
    }
    ...
    while !state.shutdown.load(Ordering::Relaxed) {
        ...
        // The simulator supplies an already-defined task orientation. Waiting for the physical
        // complementary filter would force non-policy hold writes into that reset before the
        // first action. This bypass is confined to `--sim-eval`; the physical path is unchanged.
        let imu_warm = sim_eval || safety.imu_ready();
        ...
        let (mut targets, gain, moving, policy_label, mut policy_trace) =
                ...
                (true, Some(sensors)) => {
                    ...
                    match controller.step(sensors, &command, snapshot.pose.active, dt, scale_mult) {
                        Ok(step) => (
                            ...
                            step.label,
                            sim_eval.then(|| proto::PolicyTraceState {
                                tick: state.ticks.load(Ordering::Relaxed),
                                observation: step.observation.to_vec(),
                                raw_action: step.raw_action.to_vec(),
                                unfiltered_targets: step.unfiltered_targets.to_vec(),
                                filtered_targets: step.targets.to_vec(),
        ...
        match safety.apply(targets, hold, gain) {
            Ok(applied) => {
                if let Some(trace) = policy_trace.as_mut() {
                    trace.applied_targets = applied.targets.to_vec();
                }
...
/// Full content identity for a policy file, logged whenever a controller loads.
///
/// A filename distinguishes slots, not weights: both the production policy and a
/// post-trained derivative can legitimately be called `alpha_stand.onnx`. The digest
/// makes a field report or A/B trial say which policy actually drove the robot.
fn policy_sha256(path: &std::path::Path) -> std::io::Result<String> {
...
mod tests {
    ...
    #[test]
    fn simulation_evaluation_requires_a_simulated_body() {
        assert!(Args::try_parse_from(["robotd", "--sim-eval"]).is_err());
        assert!(
            Args::try_parse_from(["robotd", "--sim", "127.0.0.1:7801", "--sim-eval-wait"]).is_err()
        );
        let args = Args::try_parse_from(["robotd", "--sim", "127.0.0.1:7801", "--sim-eval"])
            .expect("a simulation body makes evaluation mode valid");
        assert!(args.sim_eval);
        assert!(!args.sim_eval_wait);
        let waiting = Args::try_parse_from([
            "robotd",
            "--sim",
            "127.0.0.1:7801",
            "--sim-eval",
        ...
        .expect("an IPC-driven simulation evaluation is valid");
        assert!(waiting.sim_eval && waiting.sim_eval_wait);
    }

    #[test]
    fn policy_identity_is_the_full_content_digest() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("policy.onnx");
        std::fs::write(&path, b"abc").unwrap();
        assert_eq!(
            policy_sha256(&path).unwrap(),
