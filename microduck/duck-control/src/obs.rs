...
impl Observation {
    ...
    /// Return the fixed observation buffer for an evidence trace.
    ///
    /// The control loop itself continues to borrow through [`Self::as_slice`].
    /// This copy is used only by the explicitly enabled simulation evaluator,
    /// where binding one policy action to the exact deployed 61D input matters
    /// more than avoiding a 244-byte stack copy.
    pub fn as_array(&self) -> [f32; OBS_LEN] {
        self.data
    }
